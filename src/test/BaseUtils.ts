...
// TODO    return hashPassword(password ? password : "");
...
const loadAuthorsSeries = async (author: Author, serieses: Series[]): Promise<void> => {
    for (const series of serieses) {
        try {
            ...
            console.info(`  Reloading AuthorsSeries for Author ID ${author.id} Series ID ${series.id}`, error);
            throw error;
        }
    }
}
...
const loadAuthorsStories = async (author: Author, stories: Story[]): Promise<void> => {
    for (const story of stories) {
        try {
            ...
            console.info(`  Reloading AuthorsStories for Author ID ${author.id} Story ID ${story.id}`, error);
        }
    }
}
...
const loadAuthorsVolumes = async (author: Author, volumes: Volume[]): Promise<void> => {
    for (const volume of volumes) {
        try {
            ...
            console.info(`  Reloading AuthorsVolumes for Author ID ${author.id} Volume ID ${volume.id}`, error);
        }
    }
}
